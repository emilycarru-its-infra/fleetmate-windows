...
1. **Pipeline access to FleetMate source or release.** `example-org/fleetmate-windows`
...
2. **Signing cert** — already available: KV `intune-graph-secrets` / `EmilyCarrWindowsEnterpriseCodeSignCert`,
   CN `Enterprise Code Signing Certificate` (same block every fetch job uses).

## Edit 1 — parameter (after `sbinInstallerTag`)

```yaml
  - name: fleetmateTag
    displayName: "FleetMate release tag (example-org/fleetmate-windows)"
    type: string
    default: latest
```
        ...
        steps:
          ...
          - pwsh: |                   # get FleetMate source at the release tag (NEEDS the token from Prereq 1)
              git clone --depth 1 --branch '${{ parameters.fleetmateTag }}' `
                "https://$(GH_TOKEN)@github.com/example-org/fleetmate-windows.git" src
